...
\textbf{Revision.}
We revised RQ1 to focus on the intended remote psychological/psychophysiological task use case: whether Camerala can obtain task-synchronized face-derived records, including rPPG-oriented ROI values, that satisfy an operational landmark-availability criterion and remain usable for descriptive analysis under the tested configuration.
The revised manuscript defines concrete operational criteria at the trial and window levels: RT 100--1500 ms for trial-level retention and \texttt{mean\_quality} $\geq$ 0.8 for window-level landmark availability.
...
\textbf{Key revised text.}
``For RQ1, we evaluated whether the exported records met operational criteria at the trial and window levels. At the trial level, trials were retained for descriptive analysis when reaction times fell within 100--1500 ms. At the window level, landmark availability was evaluated using \texttt{mean\_quality}, the mean of the binary landmark-availability flag within each 10-s window; windows with \texttt{mean\_quality} $\geq$ 0.8 were treated as satisfying the operational landmark-availability criterion.''
...
Thus, \texttt{mean\_quality} $\geq$ 0.8 means that landmarks were available in at least 80\% of frames in that window.
