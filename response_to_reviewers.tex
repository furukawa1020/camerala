...
\textbf{Revision.}
We revised RQ1 to focus on the intended remote psychological/psychophysiological task use case: whether Camerala can export task-synchronized face-derived records, including rPPG-oriented ROI values, with exported windows satisfying an operational landmark-availability criterion and remaining usable for descriptive analysis under the tested configuration.
The revised manuscript defines concrete operational criteria at the trial and exported-window levels: RT 100--1500 ms for trial-level retention and \texttt{mean\_quality} $\geq$ 0.8 for landmark availability within exported frame/window records.
