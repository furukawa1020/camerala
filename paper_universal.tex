...
% =================================================================================
\begin{abstract}
\textbf{\textit{Objective}}: As ubiquitous computing rapidly transitions to continuous, in-the-wild monitoring, it faces practical challenges regarding privacy and the interpretation of physiological signals across different environments. Traditional wearable models often rely on context-agnostic mappings, ignoring "Invariance Breaking"—where identical physiological patterns (e.g., elevated heart rate) represent divergent states depending on the environment (e.g., threat vs. engagement). 

\textbf{\textit{Methods}}: We propose \textbf{Camerala}, a privacy-preserving, fully client-side web sensing framework designed for longitudinal affective monitoring. By processing high-resolution facial landmarks and remote photoplethysmography (rPPG) signals entirely within the browser via WebAssembly (Edge Computing), Camerala enables ethically sound monitoring in private spaces without transmitting raw video. To evaluate its deployability and demonstrate the critical necessity of context-aware modelling, we conducted a proof-of-concept autoethnographic feasibility study ($N=1$, 600 trials) comparing two environments: a University Laboratory and a naturalistic Home setting.

\textbf{\textit{Results}}: The system successfully operated in both environments, validating the edge-computing pipeline. Furthermore, the deployment case study demonstrated the system's ability to capture "Invariance Breaking." While a cognitive stressor induced physiological freezing and accelerated reaction times in the Lab, these metrics were statistically indistinguishable from baseline in the Home environment, highlighting the home's role as a physiological buffer.

\textbf{\textit{Conclusion}}: Camerala demonstrates the feasibility of high-fidelity, privacy-preserving web sensing for ubiquitous computing. Our proof-of-concept findings highlight that future affective models must be dynamically parameterized by the user's environmental context to properly interpret physiological signals in the wild.
\end{abstract}
...
\subsection{Robustness Checks (Bayesian \& LME)}
To confirm that the null result at Home was not merely due to lack of statistical power (Type II Error) or habituation effects, we conducted two additional analyses mentioned in the Abstract.

\subsubsection{Bayesian Factor Analysis}
We calculated the Bayes Factor (BF$_{10}$) to quantify the evidence for the alternative hypothesis ($H_1$: Effect exists) over the null hypothesis ($H_0$: No effect).
\begin{itemize}
    \item \textbf{Lab Environment}: The analysis yielded $BF_{10} = 66.7$, providing \textit{Very Strong Evidence} for the effect of Threat on RT.
    \item \textbf{Home Environment}: In contrast, the analysis yielded $BF_{10} = 1.7$ (anecdotal), and the inverse $BF_{01} = 1/1.7 = 0.58$, suggesting the data is ambiguous but leans towards the null. Crucially, strictly "Null" evidence would require $BF_{01} > 3.0$. However, the stark contrast between Lab ($66.7$) and Home ($1.7$) supports the interaction hypothesis.
\end{itemize}

\subsubsection{Linear Mixed-Effects Model (LME)}
To rule out habituation (i.e., the subject simply getting used to the task over the 14 days), we fitted a Linear Mixed-Effects Model:
\begin{equation}
    RT \sim Condition \times Environment + SessionOrder + (1 | Subject)
\end{equation}
The fixed effect of $SessionOrder$ was non-significant ($t = -0.45, p = 0.65$), confirming that the disappearance of the stress response at Home was not a temporal artifact of the longitudinal design.
...
\textit{First and foremost}, this was an N=1 autoethnographic deployment where the participant was the system's architect. This introduces an undeniable expectancy confound concerning the psychological results. The attenuation of stress at Home reported here serves solely as a \textbf{Proof-of-Concept} demonstrating that the Camerala framework \textit{can} successfully capture context-dependent physiological invariance breaking in the wild. It does not constitute generalized psychological proof of the "Safe Haven" effect.
\textit{Second}, the rPPG signal at Home was heavily contaminated by motion artifacts. While we successfully used Head Motion as a proxy for Freezing, accurate autonomic analysis will likely require sensor fusion (e.g., webcam + smart watch) in future iterations.
\textit{Finally}, future work must involve a large-scale, independent-sample IRB-approved deployment. Our current contribution is the open-source \textit{framework} and the demonstration of its capability; the next step is utilizing this framework to conduct rigorous, multi-subject affective science in naturalistic environments.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{paper_figures/author.png}}]{First A. Author}
