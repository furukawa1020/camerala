...
| 5 | Justify novelty and positioning | Add related work on existing web/edge rPPG systems; explicitly state novelty is integrated task execution + on-device features + timestamped local logging, not local rPPG alone. | List added references and quote positioning paragraph. | Drafted; OA references only |
