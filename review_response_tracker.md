...
| 7 | Clarify concrete usage scenario | Define Camerala's target use case as remote psychological/psychophysiological task experiments conducted in private or home settings, where the participant sits in front of a laptop, views browser-based task stimuli, and responds through the browser. Clarify that the goal is not continuous physiological sensing during ordinary daily activities, and that the laboratory setting in this deployment is a comparison context rather than the primary target scenario. | Quote the revised Introduction and Methodology usage-scenario paragraphs. | Drafted |
| 8 | Define/qualify "in-the-wild" and "naturalistic" | Remove unqualified "in-the-wild" and "naturalistic" expressions; define the target use case as remote psychological/psychophysiological task experiments in private or home settings, and explicitly state that Camerala is not intended for continuous physiological sensing during ordinary daily activities. Treat the laboratory setting as a comparison context in the present exploratory deployment. | Show risky-term search and quote revised scope statement. | Drafted |
