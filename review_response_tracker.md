...
| 5 | Justify novelty and positioning | Use peer-reviewed open-access sources for online webcam rPPG, open-source rPPG, efficient/mobile camera-based sensing, rPPG toolboxes, and privacy-related signal removal; state novelty is integrated task execution + on-device features + timestamped local logging, not browser rPPG or local rPPG alone. | List added OA sources; explain Labvanced/FacePhys-Demo are public tools/demos, not peer-reviewed evidence; quote positioning paragraph. | Drafted; strict OA bibliography, with MediaPipe arXiv only as a technical implementation reference |
