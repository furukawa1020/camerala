...
\etitle{Camerala: An Edge-Side, Browser-Native Framework for Remote Psychophysiological Experiments with rPPG Sensing}
...
\begin{abstract}
Camerala is an edge-side, browser-native framework for remote psychophysiological experiments with rPPG sensing.
...
An exploratory N=1 author-participant deployment was evaluated as a 10-session, 600-trial browser-based task experiment in laboratory and home settings.
The deployment was assessed against a study-specific exploratory 80\% behavioral-response reference level and temporally alignable export of task-event and face-image-derived records.
...
These findings are limited to the tested author-participant and configuration and do not establish general feasibility, robustness, validity in other settings, rPPG accuracy, or psychological/physiological effects.
\end{abstract}
...
To address this deployment scenario, we present \textbf{Camerala}, an edge-side, browser-native framework for remote psychophysiological experiments with rPPG sensing.
...
\begin{itemize}
    \item \textbf{RQ1:} In the tested remote psychological/psychophysiological task setting, did Camerala record behavioral responses meeting the study-specific reaction-time criterion in at least 80\% of completed trials while exporting the task-event and rPPG-sensing records in a temporally alignable form?
...
\texttt{mean\_quality} is reported only as a descriptive exported-window-level landmark-availability summary; it is not used as the main RQ1 criterion and should not be interpreted as face-detection availability across all camera frames.
...
\begin{enumerate}
    ...
    \item \textbf{Task-synchronized feature logging pipeline}: A logging pipeline that records task events, rPPG-oriented ROI values, FaceMesh-derived motion estimates, exported-record landmark summaries, and frame-derived brightness-change indicators without raw facial video persistence.
    \item \textbf{Exploratory deployment under limited conditions}: An N=1 author-participant deployment used to assess the study-specific operational criteria under limited laboratory/home conditions.
\end{enumerate}
...
However, they do not generally provide an integrated webcam-derived physiological feature pipeline, task-sensing synchronization, and client-side feature logging as a single browser runtime.
...
\textbf{Camerala} (derived from "Camera" + "Laboratory") is positioned as an edge-side, browser-native research implementation for remote psychological and psychophysiological task experiments.
Its contribution is not browser-based rPPG, local rPPG processing, or privacy preservation as an algorithm.
Rather, it integrates task execution, face-derived feature extraction, shared-clock task/frame timestamps, exported-record landmark summaries, and local CSV/ZIP persistence without raw facial video retention within one browser runtime.
...
All 10 planned sessions and 600 trials were completed.
...
Descriptively, all 299 exported windows had \texttt{mean\_quality} = 1.0; this exported-window summary was not used as the primary evidence for RQ1.
...
\midrule
Trials within RT range & 256/300 (85.3\%) & 285/300 (95.0\%) & Trial-level; RT 100--1500 ms \\
...
The study does not claim clinical or medical accuracy, general deployability, general feasibility, or validated cognitive, motivational, psychological, or physiological effects; it presents an implementation example of browser-native, task-aligned, client-side feature logging under limited laboratory and home conditions.

\section{Conclusion}
This paper presented Camerala as an edge-side, browser-native framework for remote psychophysiological experiments with rPPG sensing.
