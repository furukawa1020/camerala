...
Differences between laboratory and home records are reported as deployment-specific sensor-derived observations, not as evidence of psychological, physiological, cognitive, motivational, or rPPG-accuracy effects.
\end{abstract}
...
Remote psychological and psychophysiological task experiments often require synchronized behavioral task logs and webcam-derived feature records while avoiding unnecessary handling of privacy-sensitive facial video.
...
To address this deployment scenario, we present \textbf{Camerala}, a browser-native framework for task-aligned physiological feature logging in remote task experiments conducted in laboratory and home settings.

\subsection{Research Questions}
The exploratory Research Questions (RQs) focus on operational completion and descriptive sensor-derived observations under the tested configuration:
\begin{itemize}
    \item \textbf{RQ1:} Under the tested laptop, browser, camera, and task configuration, can Camerala complete the planned 10-session, 600-trial remote task deployment and export timestamp-linked local records that remain usable for descriptive analysis after a specified reaction-time criterion?
    \item \textbf{RQ2:} What differences in task-synchronized sensor-derived records are observed between the laboratory and home deployment contexts, and how should these differences be interpreted given uncontrolled environmental and measurement factors?
\end{itemize}
...
\begin{enumerate}
    ...
    \item \textbf{Exploratory remote task deployment in laboratory and home settings}: A limited N=1 author-participant deployment showing completion of a remote task procedure in laboratory and home settings, with local export of task-synchronized sensor-derived records.
\end{enumerate}
...
The logged \texttt{quality} field was treated as a landmark-availability flag for saved records, not as a continuous face-detection or tracking-confidence value.
...
These differences are treated as deployment-specific sensor-derived observations.
...
The appropriate interpretation is that the tested deployment yielded sensor-derived records whose values differed descriptively between the tested laboratory and home contexts.
...
\subsection{Quality Indicators for Laboratory and Home Deployment Contexts}
...
The laboratory/home differences should be interpreted cautiously because deployment context, lighting, posture, camera geometry, task timing, and participant state were not independently controlled.
...
Sixth, the current logs do not store continuous face-detection or tracking-confidence values for all frames.
...
The present study presents an implementation example of browser-native, task-aligned, local-first feature logging under limited laboratory and home conditions.
