...
\begin{itemize}
    \item \textbf{RQ1:} In the intended remote psychological/psychophysiological task use case, can Camerala obtain task-synchronized face-derived records, including rPPG-oriented ROI values, that satisfy the operational landmark-availability criterion of \texttt{mean\_quality} $\geq$ 0.8 and remain usable for descriptive analysis under the tested configuration?
...
For RQ1, we evaluated whether the exported records met operational criteria at the trial and window levels.
At the trial level, trials were retained for descriptive analysis when reaction times fell within 100--1500 ms.
At the window level, landmark availability was evaluated using \texttt{mean\_quality}, the mean of the binary landmark-availability flag within each 10-s window; windows with \texttt{mean\_quality} $\geq$ 0.8 were treated as satisfying the operational landmark-availability criterion.
...
Thus, the reported retained-trial count reflects the reaction-time criterion, while the landmark-availability criterion was satisfied by all exported windows.
These results describe the available analyzed trials and windows under the tested configuration and do not demonstrate general feasibility across devices, users, or environments.
