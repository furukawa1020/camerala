...
\begin{itemize}
    \item \textbf{RQ1:} In the tested remote psychological/psychophysiological task setting, did Camerala record behavioral responses in at least 80\% of completed trials while exporting the task-event and rPPG-sensing records in a temporally alignable form?
...
\begin{enumerate}
    \item \textbf{High-resolution timestamps}: Trial starts, block events, and frame features use \texttt{performance.now()} timestamps.
...
\WHILE{Session Active}
    ...
    \FOR{trial $i = 1$ to $n$}
        ...
        \STATE $t_{stim} \leftarrow$ performance.now()
        \WHILE{No KeyPress}
            \STATE Poll F/J KeyPress
        \ENDWHILE
        \STATE $t_{key} \leftarrow$ performance.now()
        \STATE $RT \leftarrow t_{key} - t_{stim}$
