...
This study has strict limitations: it was an exploratory N=1 deployment with the author as participant; no reference physiological sensor, ECG, or contact PPG was collected; one primary laptop/camera configuration was used; and multiple devices, multiple participants, and multiple home environments were not tested.
Lighting, screen light, posture, camera placement, task timing, participant state, and task context were not experimentally separated, and no comparison with existing experiment runtimes, browser rPPG tools, or cloud APIs was conducted.
The current implementation does not correct motion-induced rPPG degradation, and future work should evaluate motion handling with reference physiological sensors, controlled posture/camera conditions, and predefined artifact-handling rules.
The current logs also do not store continuous confidence values from the face-processing pipeline for all frames; therefore, failed landmark-detection frames and overall tracking-success rates cannot be recovered.
...
The study does not claim clinical or medical accuracy, general deployability, general feasibility, or validated cognitive, motivational, psychological, or physiological effects.
