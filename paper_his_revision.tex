...
This design supports timestamp alignment within the same browser runtime without raw-video persistence.
...
\begin{itemize}
    \item \textbf{RQ1:} Under the tested laptop, browser, camera, and task configuration, can Camerala complete the planned 10-session, 600-trial remote task deployment and export timestamp-linked local records that satisfy the reaction-time criterion of 100--1500 ms and the window-level landmark-availability criterion of \texttt{mean\_quality} $\geq$ 0.8 for descriptive analysis?
...
For RQ1, we examined completion of the planned 10 sessions and 600 trials, successful local CSV/ZIP export, timestamp linkage between task events and saved frame/window records, trial retention after applying the reaction-time criterion of 100--1500 ms, and window-level landmark availability.
The landmark-availability criterion was defined using \texttt{mean\_quality}, the mean of the binary landmark-availability flag within each 10-s window; windows with \texttt{mean\_quality} below 0.8 were treated as below the operational landmark-availability criterion.
...
For each processed frame, Camerala logged a binary landmark-availability flag, \texttt{quality}, where 1 indicates that FaceMesh landmarks were returned and 0 indicates that no landmarks were returned.
For each 10-s window, \texttt{mean\_quality} was computed as the mean of this binary flag.
Thus, \texttt{mean\_quality} represents the proportion of frames in that window for which landmarks were available.
...
All 299 exported windows had \texttt{mean\_quality} = 1.0 and therefore satisfied the operational landmark-availability criterion of 0.8; no window was excluded by this criterion.
...
Sixth, the current logs do not store continuous confidence values from the face-processing pipeline for all frames.
