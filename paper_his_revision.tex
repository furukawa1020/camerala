...
\begin{itemize}
    \item \textbf{RQ1:} In the intended remote psychological/psychophysiological task use case, can Camerala export task-synchronized face-derived records, including rPPG-oriented ROI values, with exported windows satisfying the operational landmark-availability criterion of \texttt{mean\_quality} $\geq$ 0.8 and remaining usable for descriptive analysis under the tested configuration?
...
At the window level, landmark availability was evaluated for exported frame records using \texttt{mean\_quality}, the mean of the binary landmark-availability flag within each 10-s window; exported windows with \texttt{mean\_quality} $\geq$ 0.8 were treated as satisfying the operational landmark-availability criterion.
